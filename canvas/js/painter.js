                ...
                for(y in options[x]){
                    if(y in lOptions[x]){
                        ...
                        if(y == "alignment"){
                            ...
                            let defs =  JSON.parse(JSON.stringify($.fn.addPainterDefaultsOptions.toolbar.alignmentProcessed));
                            while(ops.length > 0){
                                for(z in defs){
...
class Painter{
    ...
    CreateEnviroment(){
        ...
        if(this.options.type == "dx-scrollview"){
            ...
            this.contentWrap = $("#"+this.ids.domId).find("div").toArray().find((x)=>{return x.getAttribute("class") == "dx-scrollview-content"});
            $(this.contentWrap).html(`<div id="${this.ids.middlemanId}">${$(this.contentWrap).html()}</div>`);
            $("#"+this.ids.domId).append(`<div id="${this.ids.editOptionsId}"></div>`);
        }
        else if (this.options.type == "div" || this.options.type == "img"){
            $("#"+this.ids.domId).parent().html(`<div id="${this.ids.middlemanId}"><div id="${this.ids.contentWrapId}">${$("#"+this.ids.domId).parent().html()}</div></div>`);
            $("#"+this.ids.middlemanId).append(`<button id="${this.ids.editButtonId}">Edit</button>`);
            $("#"+this.ids.contentWrapId).append(`<div id="${this.ids.editOptionsId}"></div>`);
        }
        /*
        ...
        if(this.options.type == defs.type[2])
            offset = [$("#"+this.ids.domId).parent().width() - $("#"+this.ids.domId).clientWidth, $("#"+this.ids.domId).parent().height() - $("#"+this.ids.domId).clientHeight]
        
        console.log($("#"+this.ids.domId).parent().width(), $("#"+this.ids.domId).clientWidth)

        if(this.options.toolbar.alignmentProcessed.axis == defs.toolbar.alignmentProcessed.axis[0]){
            // align horizontaly
            if(this.options.toolbar.alignmentProcessed.posToBorder == defs.toolbar.alignmentProcessed.posToBorder[1]){
                position.top -= $("#"+this.ids.editOptionsId).height();
                position.bottom -= $("#"+this.ids.editOptionsId).height();
            }
        }
        else if(this.options.toolbar.alignmentProcessed.axis == defs.toolbar.alignmentProcessed.axis[1]){
            // align verticaly
            if(this.options.toolbar.alignmentProcessed.posToBorder == defs.toolbar.alignmentProcessed.posToBorder[1]){
        ...
        console.log(position, [$("#"+this.ids.editOptionsId).width(), $("#"+this.ids.editOptionsId).height()], offset)
        ...
        if(this.options.toolbar.alignmentProcessed.posX == defs.toolbar.alignmentProcessed.posX[0])
            $("#"+this.ids.editOptionsId).css("left", position.left);
        else if(this.options.toolbar.alignmentProcessed.posX == defs.toolbar.alignmentProcessed.posX[1])
            $("#"+this.ids.editOptionsId).css("right", position.right);

        if(this.options.toolbar.alignmentProcessed.posY == defs.toolbar.alignmentProcessed.posY[0])
            $("#"+this.ids.editOptionsId).css("top", position.top);
        else if(this.options.toolbar.alignmentProcessed.posY == defs.toolbar.alignmentProcessed.posY[1])
        ...
        */
        ...
        $("#"+this.ids.middlemanId).append(`<div id="${this.ids.canvasWrapId}" class="canvas-wrap">${canvas}</div>`);
    }
    ...
    ResizeCanvas(element){
        let size;
        if(this.options.type == "img"){size = [element.clientWidth, element.clientHeight]}
        else{size = [element.width(), element.height()]}
